...
*/
void createIndexFile(FILE *input, char *dataMemberName, char *indexType, char *nameIndexFile) {
    ...
    IndexData *arr = createIndexArr(input, idxHeader, dataMemberName);

    // writes index file after sorting index arr data
    writeFileIndex(index, arr, idxHeader, dataMemberName);
    fclose(index);
}

/*
* This function receives a .bin file input with registers
* and a .bin index file named 'nameIndexFile', indexed by 'dataMemberName'
* It searches numberSearches registers and prints its data
* It priorizes searching by index (if possible)
*/
void searchInBinaryFile(FILE *input, char *dataMemberName, char *indexType, char *nameIndexFile, int numberSearches) {
    ...
    IndexData *indexDataArr = readFileIndex(indexFile, dataMemberName, indexHeader);
    fclose(indexFile);

    for (int i = 0; i < numberSearches; i++) {
        Search *search = createSearchArr(input);
        Result *res = superSearch(input, dataMemberName, indexDataArr, indexHeader, search, dataHeader);
...
*/
void deleteRegister(FILE *input, char *dataMemberName, char *indexType, char *nameIndexFile, int numberDeletions) {
    // reading data header
    Header *dataHeader = readHeaderBinary(input);
    if (!dataHeader) return;
    ...
    writeFileIndex(indexFile, indexDataArr, indexHeader, dataMemberName);
    fclose(indexFile);
...
*/
void insertRegister(FILE *input, char *dataMemberName, char *indexType, char *nameIndexFile, int numberInsert) {
    ...
    for (int i = 0; i < numberInsert; i++) {
        ...
        if (isIntegerMember(dataMemberName)) {
            indexDataArr = appendIndexArray(indexDataArr, indexHeader, dataMemberName, selectIntegerMember(dataMemberName, reg), NULL, currentByteOff);
        }
        else {
            indexDataArr = appendIndexArray(indexDataArr, indexHeader, dataMemberName, -1, selectStrMember(dataMemberName, reg), currentByteOff);
        }
    }
...
*/
void updateRegister(FILE *input, char *dataMemberName, char *indexType, char *nameIndexFile, int numUpdates) {
    // reading data header
    Header *dataHeader = readHeaderBinary(input);
    // switching the status to inconsistent
    updateHeader(input, dataHeader);
    ...
    for (int i = 0; i < numUpdates; i++) {
        ...
        indexDataArr = superUpdate(input, update, res, indexDataArr, indexHeader, dataHeader, dataMemberName);
    }
...
void searchInTree(FILE *dataFile, char *dataMemberName, char *nameIndexFile, int numberSearches) {
    ...
    for (int i = 0; i < numberSearches; i++) {
        ...
        // if we are looking for a member from the data register that is a string we gotta use seq search
        if (!isIntegerMember(dataMemberName)) {
